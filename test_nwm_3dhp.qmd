...
aoi <- st_as_sfc(st_bbox(lines_3dhp))
sf_nhdp_wa_aoi <- sf_nhdp_wa[aoi,]
ggplot() + geom_sf(data = sf_nhdp_wa_aoi, fill = NA) + geom_sf(data = aoi, color = "purple", fill = NA)

#not bad, maybe ~1min?
lines_3dhp <- st_join(
  lines_3dhp,
  sf_nhdp_wa_aoi |> select(-state),

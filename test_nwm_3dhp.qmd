...
# #this 'rooted forest' has GIS artifact strays
# ggplot() +
...
#can likely pipe this directly into original object declaration
#but for now allows qaqc of intermediates
#adds a calc'd topological distance for nodes which then associated to 'to' of edges
#appears that the ground distance versions are 'from downstream node to downstream [root] node'
lsfn <- lsfn |>
  tidygraph::activate(nodes) |> 
  tidygraph::convert(
  ...
  mutate(
    idn = seq_along(SHAPE),
    d_to_root = tidygraph::node_distance_to(tidygraph::node_is_root()),
    d_to_root_ft = tidygraph::node_distance_to(tidygraph::node_is_root(), weights = shape_length),
    d_to_root_km = 0.0003 * d_to_root_ft
    ) 

# #idn is 'to' of root edge == 892, edge uid should be 8004
# #but idn != .tidygraph_node_index (created before the convert), 892 vs 900 for dtoroot0 
# lsfn |> st_as_sf("edges") |> sf::st_zm() |> mapview::mapview()
# lsfn |> st_as_sf("nodes") |> as_tibble() |> select(-SHAPE) |> arrange(d_to_root)

lsfn <- lsfn |> 
  tidygraph::activate(edges) |> 
  left_join(
    lsfn |> st_as_sf("nodes") |> as_tibble() |> select(-SHAPE)
    ,
    by = c("to" = "idn")
    )
...
ggplot() + 
  geom_sf(
    data = lsfn |> st_as_sf("nodes") 
    ,
    color = "grey50", alpha = 0.5) +
  geom_sf(
    data = lsfn |> st_as_sf("edges") |> 
      filter(d_to_root == 0)
    , linewidth = 1.5, color = "orange"
  )

ggplot() + 
  #geom_sf(data = lsfn |> st_as_sf("edges"), aes(color = d_to_root)) + 
  geom_sf(data = lsfn |> st_as_sf("edges"), aes(color = d_to_root_km)) + 
...
Next - think about reverse travel time to get a cfs-unit value 'up'

But really need to progress iteratively from network perimeter down?

```{r}
#super dumb demo of 'spreading a fixed volume up network'?
#decaying by d_km at ...power-law rate?
#need to get a cumulative drainage area not just root distance
#so for each node, get 'upstream' distance to furthest leaf
#the diameter of the subgraph above a given node?
#may also need to ensure directed to root?
tidygraph::with_graph(lsfn, tidygraph::graph_is_dag()) #true, good

lsfn |> 
  tidygraph::activate(nodes) |> 
  mutate(
    # #this (unsurprisingly) does not work: 
    # d_to_perim = tidygraph::node_distance_to(tidygraph::node_is_leaf())
    # # neither this...
    # d_to_perim = tidygraph::node_max_flow_from(tidygraph::node_is_leaf(), capacity = shape_length)
  )

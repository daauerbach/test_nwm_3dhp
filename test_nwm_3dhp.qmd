...
# y <- sfn |> 
#   tidygraph::activate("edges") |> 
#   filter(zslope_est > slope_threshold) 
# #yes. better. still gets rid of steep fringes and/but creates 767 components
# yy <- y |> tidygraph::convert(
#   tidygraph::to_largest_component, .clean = T
#   )
# yy |> filter(uid == root_edge_uid)
# 
# list(
#   yy = as_tibble(yy, "edges") |> st_zm(),
#   y = as_tibble(y, "edges") |> st_zm(),
#   full = as_tibble(sfn, "edges") |> st_zm()
# ) |> mapview::mapview(color = c("purple","green","blue"))
...
## ordinate culverts by root dist & upstream extent & slope & length
## slope_threshold filter creates some stranded perimeter 'below_culv' nodes
anti_join(
  as_tibble(as_tibble(sfn, "nodes")) |> filter(node_below_culvert)  #983
  ,
  as_tibble(as_tibble(sfn, active = "edges")) |> filter(str_detect(desc, "ulvert")) #955
  ,
  by = c("idn" = "to")
) #31

as_tibble(as_tibble(sfn, "nodes")) |> count(node_below_culvert)
#confirming that the 31 TRUE below_culvert nodes are at perimeter having had their upstream edges, at least one of which was culvert, cut off
list(
  edge = as_tibble(sfn, "edges") |> st_zm(),
  node = as_tibble(sfn, "nodes") |> st_zm(),
  # foo = anti_join(
  #   as_tibble(sfn, "nodes") |> filter(node_below_culvert),
  #   as_tibble(as_tibble(sfn, active = "edges")) |> filter(str_detect(desc, "ulvert")),
  #   by = c("idn" = "to")
  # ) |> st_zm()
  foo = as_tibble(sfn, "nodes") |> slice(415) |> st_zm()
) |>
  mapview::mapview(zcol = list("length_ft", "node_below_culvert", "d_root"))


#if topology attributes added back to edges, could also have edge/reach/culvert length
#not associated to nodes themselves 
#maybe better to overhaul/re-do treating edges as nodes? sfn |> tidygraph::convert(tidygraph::to_linegraph) 

sfn_culv <- as_tibble(sfn, active = "edges") |> 
  filter(str_detect(desc, "ulvert")) |> 
  left_join(
    as_tibble(as_tibble(sfn, "nodes")) |> 
      select(to = idn, 
             #node_below_culvert, 
             starts_with("d_"), 
             in_ft_all, in_km_all, in_ft_pct #starts_with("in_")
             )
    ,
    by = "to"
  )

sfn_culv |> st_zm() |> st_centroid() |> 
  mapview::mapview(zcol = "in_km_all")

#interesting/useful, and/but note the outlier high in_km is uid 64871 Lake Creek with Lk Cavanaugh and all its tribs above
#such that it could indeed be a critical culvert...or not, depending on expectations for through lake mvmt
plotly::ggplotly(
as_tibble(sfn_culv) |> 
  ggplot() + 
  #geom_point(aes(d_root_km, zslope_est, color = in_km_all, size = in_km_all)) +
  #geom_point(aes(d_root_km, in_km_all, color = abs(zslope_est), size = abs(zslope_est))) +
  geom_point(aes(d_root_km, in_km_all, color = abs(zslope_est), size = length_ft), alpha = 0.5) 
#+ scale_size_binned_area(max_size = 4) + scale_color_binned()
)

#need some 'marginal habitat gain' for optimization
#calc distance to next upstream culvert (or end of line if none)?
#could do all-to-all then determine nearest then use that to index out of all-all object
#this works, https://luukvdmeer.github.io/sfnetworks/reference/st_network_cost.html

#direction "in" defines distances from upstream nodes
#for first row (root node) equivalent to d_root_ft
d_in <- sfnetworks::st_network_cost(sfn, direction = 'in', weights = as_tibble(sfn, "edges")$length_ft)
d_in[1:10,1:10]
...
apply(d_in[sfn_culv$to, sfn_culv$to], 1, \(x) sort(x)[2])
which(sfn_culv$to==10) #index 415 is node 10: sfn_culv$to[415]
apply(d_in[sfn_culv$to, sfn_culv$to], 1, \(x) sort(x)[2])[415] #9104 ~ 69+2793+267+975+2218+1839+76+197+668
apply(d_in[sfn_culv$to, sfn_culv$to], 1, \(x) sort(x)[2])[which(sfn_culv$to==76)] #466 ~ 33.25+267+165.6
apply(d_in[sfn_culv$to, sfn_culv$to], 1, \(x) sort(x)[2])[which(sfn_culv$to==121)] #207 ~ 66.3+140.8
apply(d_in[sfn_culv$to, sfn_culv$to], 1, \(x) sort(x)[2])[which(sfn_culv$to==205)] #Inf as it should be with no higher culverts
...
d_out <- sfnetworks::st_network_cost(sfn, direction = 'out', weights = as_tibble(sfn, "edges")$length_ft)
d_out[1:10,1:10]
apply(d_out[sfn_culv$to, sfn_culv$to], 1, \(x) sort(x)[2])

#trying to get the subgraph of shortest paths from (lowest) culverts with none below to root
lowest_culv <- sfn_culv$to[which(apply(d_out[sfn_culv$to, sfn_culv$to], 1, \(x) sort(x)[2])==Inf)]
lowest_culv_paths <- sfnetworks::st_network_paths(sfn, from = 1, to = lowest_culv, mode = 'in')
...
ggplot() +
  geom_sf(data = as_tibble(sfn, "edges"), color = "lightblue") +
  geom_sf(data = as_tibble(sfn_lowest_culv, "edges"), color = "darkgreen")

list(
  edge = as_tibble(sfn, "edges") |> st_zm(),
  node = as_tibble(sfn, "nodes") |> st_zm(),
  lowculv = as_tibble(sfn_lowest_culv, "edges") |> st_zm()
) |>
  mapview::mapview(zcol = list("length_ft", "node_below_culvert", "length_ft"))

# #something more elegant? https://luukvdmeer.github.io/sfnetworks/articles/sfn05_routing.html
# #maybe riffing off https://stackoverflow.com/questions/76115139/create-subgraph-from-shortest-path-calculation-sfnetworks-r
# #could not get tidygraph::node_distance_from mapping correctly
# #this is correct but slow as-is, likely easy speed up at least from declaring weight vector standalone
# sfn |> 
#   mutate(
#     d_culv_up = map_dbl(
#       idn,
#       ~sort(sfnetworks::st_network_cost(sfn, from = sfn_culv$to, to = .x, direction = 'out', weights = as_tibble(sfn, "edges")$length_ft))[2]
#     )
#   ) 

#for now use the distance to the nearest upstream culvert or all upstream length if no culverts above
#non-culvert nodes need first position of sort: apply(d_in[, sfn_culv$to], 1, \(x) sort(x)[1])
sfn_culv <- sfn_culv |> 
  mutate(
    ...
    ,
    psuedocost = abs(length_ft * as.vector(zslope_est))
  )

```

try for something derived from  `solve_opt` from https://github.com/sljardine/upstream/blob/main/upstream/R/fct_Suggest.R
...
#start with unweighted hab: obj==1 s.t. points$hmarg -> h -> v, 
#with points sf df with: "hmarg_length, hmarg_area, hmarg_volume, cost, and wria_number"
#inspecting D.rda from repo, 'connectivity matrix' means binary but need to follow up with Sunny on exactly how generated
#here, 1 if culvert-i (row) has a non-infinite downstream distance to culvert-j (col)
#note need: install.packages("ROI.plugin.glpk")
library(ROI.plugin.glpk)
ROI::ROI_registered_solvers()
#so this runs but 'sol' is allNA and throws "PROBLEM HAS NO PRIMAL FEASIBLE SOLUTION"
v <- sfn_culv$hmarg
D <- 1*!is.infinite(d_out[sfn_culv$to, sfn_culv$to])
brc <- sfn_culv$psuedocost
nb <- length(v)
di <- colSums(D)
budget <- 20 #plot(density(brc)); quantile(brc, p = seq(0,1, by = 0.1))
obj <- ROI::L_objective(v)
bc <- ROI::L_constraint(L = brc, dir = "<=", rhs = budget) #budget
hc <- ROI::L_constraint(L = diag(nb) - t(D), dir = rep("<=", nb), rhs = 1 - di) #hydrology
prob <- ROI::OP(objective = obj,
                ...
                types = rep.int("B", nb),
                maximum = TRUE)
sol <- ROI::ROI_solve(prob, solver = "glpk",  control = list("verbose" = TRUE, "presolve" = TRUE))
soln <- as.logical(ROI::solution(sol))
...
# examine Brinkerhoff et al. 2024

```{r first_look}
#from https://apps.nationalmap.gov/downloader/ via e.g. 'https://prd-tnm.s3.amazonaws.com/StagedProducts/Hydrography/NHDPlusHR/VPU/Current/GDB/NHDPLUS_H_1703_HU4_GDB.zip'

#1708 is Lwr Col; IMW basins are evident in HUC12s
st_read(file.path(dir_data_common, "nhdphr/17/NHDPLUS_H_1708_HU4_GDB.gdb/"), layer = "WBDHU12") |> ggplot() + geom_sf()

#1703 Yakima
#1708 Lwr Col, ~200K lines
#1711 Puget Sound, ~300K lines 
st_layers(file.path(dir_data_common, "nhdphr/17/NHDPLUS_H_1711_HU4_GDB.gdb/"))
sf_nhdphr_h12 <- st_read(file.path(dir_data_common, "nhdphr/17/NHDPLUS_H_1711_HU4_GDB.gdb/"), layer = "WBDHU12")
sf_nhdphr_h10 <- st_read(file.path(dir_data_common, "nhdphr/17/NHDPLUS_H_1711_HU4_GDB.gdb/"), layer = "WBDHU10")
sf_nhdphr_h08 <- st_read(file.path(dir_data_common, "nhdphr/17/NHDPLUS_H_1711_HU4_GDB.gdb/"), layer = "WBDHU8")

sf_nhdphr_flw <- st_read(file.path(dir_data_common, "nhdphr/17/NHDPLUS_H_1711_HU4_GDB.gdb/"), layer = "NHDFlowline") |> 
  left_join(
    read_csv(file.path(dir_data_common, "nhdphr/brinkerhoffetal2024_results_archive/results_1711.csv"))
    ,
    by = "NHDPlusID") |> 
...
# # drop_na based on check of object without it
# # ~9.5k flowlines missing percent ephemeral 
# # mostly shorelines/canals etc.
# sf_nhdphr_flw |> filter(is.na(percQEph_reach)) |> ggplot() + geom_sf()

#cut down to Deschutes R
aoi <- sf_nhdphr_h12 |> 
  #as_tibble() |> 
  filter(States == "WA") |> 
  select(HUC12, Name, AreaSqKm) |> 
  #filter(str_detect(Name, "Still"))
  filter(str_detect(Name, "Deschut")) #|> mapview::mapview()

aoi_flw <- sf_nhdphr_flw |> 
  #as_tibble() |> #glimpse()
  mutate(h8 = str_sub(ReachCode, 1, 8)) |> 
  filter(h8 == unique(str_sub(aoi$HUC12, 1,8))) |> 
  st_cast("LINESTRING") 
...
aoi_flw |> 
  as_tibble() |> 
  ggplot() + geom_boxplot(aes(perenniality, percQEph_reach), varwidth = T, outliers = F)

mapview::mapview(aoi) +
  mapview::mapview(aoi_flw, zcol = "percQEph_reach") 

#as suggested in paper & supp, examining median at smaller spatial scale
#drives towards bimodality
sf_nhdphr_h08 |> 
  left_join(
    sf_nhdphr_flw |> 
      as_tibble() |>  
      mutate(HUC8 = str_sub(ReachCode, 1, 8)) |> #distinct(HUC8) #21, matches poly count
      summarise(
        across(percQEph_reach, list(median = ~median(., na.rm = T))),
        .by = HUC8
      ) #|> print(n= Inf)
    , by = "HUC8"
  ) |> 
  ggplot() + geom_sf(aes(fill = percQEph_reach_median)) + wacolors::scale_fill_wa_c()

#to vis at H10 or H12 seems need to do spatial join...
#117 in polys but 40 derived from ReachCode and only 19 in common
inner_join(
  as_tibble(sf_nhdphr_h10) |> distinct(HUC10) |> arrange()
  ,
  as_tibble(sf_nhdphr_flw) |> mutate(HUC10 = str_sub(ReachCode, 1, 10)) |> distinct(HUC10) |> arrange()
  , by = "HUC10")

#the ~2.3K 'huc12' values derived from ReachCode line up poorly with the ~511 in the HUC12 poly
#only 87 in common; whatever the first 12 positions are, they're not the 'normal' HUC12
inner_join(
  as_tibble(sf_nhdphr_h12) |> distinct(HUC12)
  ,
  as_tibble(sf_nhdphr_flw) |> mutate(HUC12 = str_sub(ReachCode, 1, 12)) |> distinct(HUC12)
  , by = "HUC12")


#table of major rivers' percent ephemeral?
#also have NHDPlusFlowlineVAA
erom <- st_read(file.path(dir_data_common, "nhdphr/17/NHDPLUS_H_1711_HU4_GDB.gdb/"), layer = "NHDPlusEROMMA")
glimpse(erom)

#is this a fascinating plot? 
#suggests that below an average QAMA about ~1 unit, systems are either mostly E or not (bimodal 1/0)  
#but that the larger systems are 'well-blended'
#i think this is implied if not directly stated in the paper w/some of the remarks on the 'scale of assessment' and the choice of HUC4 
sf_nhdphr_flw |> 
  as_tibble() |>  
  drop_na(GNIS_Name) |> #down to ~51k 'named' flowlines
  #count(GNIS_Name) |> arrange(desc(n)) #1700 named reaches
  left_join(erom |> select(NHDPlusID, QAMA), by = "NHDPlusID") |> 
  summarise(
    across(
      c(QAMA, percQEph_reach), 
      list(median = ~median(., na.rm = T))),
    .by = "GNIS_Name"
  ) |> 
  arrange(desc(QAMA_median)) |> 
  #print(n= Inf)  
  ggplot() + 
  geom_point(aes(QAMA_median, percQEph_reach_median), size = 0.5, alpha = 0.5) +
  scale_x_log10()


sf_nhdphr_flw |> 
  #as_tibble() |> 
  select(GNIS_Name, NHDPlusID, perenniality, percQEph_reach) |> 
  drop_na(GNIS_Name) |> #down to ~51k 'named' flowlines
  left_join(erom |> select(NHDPlusID, QAMA), by = "NHDPlusID") |> 
  #pull(QAMA) |> plot.ecdf(); abline(v = 100)
  #filter(QAMA > 100) |> #100 is ~12.2K lines 
  filter(QAMA > 50) |> #100 is ~17.2K lines 
  # summarise(
  #   across(
  #     c(QAMA, percQEph_reach), 
  #     list(median = ~median(., na.rm = T))),
  #   .by = "GNIS_Name"
  # ) |> 
  # ggplot() + geom_point(aes(QAMA_median, percQEph_reach_median), size = 0.5, alpha = 0.5) + scale_x_log10()
  #slice_max(QAMA, n = 1, by = GNIS_Name) |> 
  ggplot() +
  geom_sf(data = sf_nhdphr_h10, fill = NA) +
  #geom_sf(aes(color = percQEph_reach)) + wacolors::scale_color_wa_c("vantage")
  geom_sf(aes(color = percQEph_reach, linewidth = log10(QAMA))) + wacolors::scale_color_wa_c("vantage")

```

```{r sf_dnr_hydro}
#1.3m features
sf_dnr_hydro <- st_read(file.path(dir_data_common, "wadnr_state_hydro/wchydro.gdb"), layer = "wchydro")

# sf_dnr_hydro[st_as_sfc(st_bbox(lines_3dhp)),] #31.4k
sf_dnr_hydro_3dhp <- sf_dnr_hydro[lines_3dhp_uch,] #23.9k

```

```{r nhdphr_dnr}
#NHDplus med res catchment for big jim creek, used above in default call to prep_sfn
sf_cat <- filter(sf_nhdp_wa_3dhp_fpb, comid == "24275471")


# list(
#   nhdphr = sf_nhdphr_flw_3dhp[sf_cat,] |> select(GNIS_Name, LengthKM, perenniality, percQEph_reach)  
#   ,
#   dnr = sf_dnr_hydro_3dhp[sf_cat,]
#   ,
#   `3dhp` = as_tibble(lsfn, "edges") |> st_zm()
# ) |> 
#   mapview::mapview(
#     zcol = list("perenniality", "WC_PERIOD_LABEL_NM", "desc"),
#     burst = T,
#     color = list("blue","green", "orange")
#     )


mapview::mapview(
  sf_nhdphr_flw_3dhp[sf_cat,] |> select(GNIS_Name, LengthKM, perenniality, percQEph_reach)
  ,
  zcol = "perenniality", burst = T, color = c("lightblue","darkblue") #ephemeral, non-ephem
  ) +
mapview::mapview(
  sf_dnr_hydro_3dhp[sf_cat,]
  ,
  zcol = "WC_PERIOD_LABEL_NM", burst = T, color = c("#831285","#C886E8") #Perennial, unknown
) +
mapview::mapview(
  as_tibble(lsfn, "edges") |> st_zm()
  ,
  zcol = "desc", burst = T, color = c("yellow","orange","#386276")
)


# #not yet run  
# sf_nhdphr_dnr <- st_join(
#   sf_nhdphr_flw_3dhp[sf_cat,] |> select(GNIS_Name, LengthKM, perenniality, percQEph_reach)  
#   ,
#   sf_dnr_hydro_3dhp[sf_cat,]
#   ,
...
union_3dhp <- st_zm(as_tibble(lsfn, "edges")) |> st_union()
union_dnr <- sf_dnr_hydro_3dhp[sf_cat,] |> st_union()
union_diff <- st_difference(
  union_3dhp,
  union_dnr |> st_buffer(dist = 30)
)

union_3dhp |> #163km
 union_dnr |> #54.7km
 union_diff |> #123km outside 30ft buffer
  st_length() |> units::set_units("km") |> as.numeric()


{ ggplot() +  geom_sf(data = union_dnr, color = "gold") + labs(title = "DNR Hydrography") } +
{ ggplot() + geom_sf(data = union_3dhp, color = "lightblue") + labs(title = "3DHP flowlines")} +
{
ggplot() +
  geom_sf(data = union_3dhp, color = "darkgreen") +
  geom_sf(data = union_diff, color = "lightblue") +
  labs(title = "Intersection (green) and difference (blue) between \n 3DHP flowlines and a 30ft buffer on DNR Hydro")
} + 
  plot_layout(ncol = 1)

ggsave(filename = "figures/f_bigjim_overlay.png", width = 7, height = 11, dpi = 150, bg = "white")

```

# testing SSNbler & SSN2

next thing here is modeling APPS, perhaps conditioning on HRCD?

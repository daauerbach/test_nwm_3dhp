...
#this still works but now has a different index value (from 73858 to 7131)
...
#so now...
sfn_culv <- as_tibble(sfn, "nodes") |> filter(node_below_culvert)
#if topology attributes added back to edges, could also have edge/reach/culvert length
#not associated to nodes themselves 
as_tibble(sfn_culv) |> 
  ggplot() + 
  geom_point(aes(d_root_km, in_slope_max, color = in_km_all, size = in_km_all)) +
